...
const int SLOW_SPEED = 110;  // Reduced speed for turns
const float SIDE_TURN_DISTANCE = 90; // Side sensor trigger (cm)
const int TOTAL_EDGES = 13;   // 4 corners * 3 laps + 1
const int SERVO_CENTER = 105;  // Servo center
const int SERVO_MAX_LEFT = 180; // Max left
const int SERVO_MAX_RIGHT = 40; // Max right
const float KP = 1.5;         // Gyro correction gain
const float KW = 0.8;         // Inner wall-following gain
const float KO = 0.5;         // Outer wall avoidance gain
const float TARGET_WALL_DISTANCE = 25; // Target inner wall distance (cm)
const float OUTER_WALL_THRESHOLD = 20; // Outer wall crash threshold (cm)
const unsigned long WAIT_DURATION = 1200; // 1s wait post-turn
...
float filteredDist = 40;

float getFilteredDistance(NewPing &sonar) {
const int samples = 3; // Increased for better reliability
  float sum = 0;
...
void performTurn(bool clockwise) {
  ...
  float turnTarget = startAngle + (clockwise ? -90 : 90);
...
void loop() {
  ...
  if (directionSet && !isWaiting) {
    ...
    float outerError = outerDist < OUTER_WALL_THRESHOLD ? OUTER_WALL_THRESHOLD - outerDist : 0; // Outer wall error (<20 cm)
    // DRIFTING LOGIC: Adjusts servo to drift toward/away from inner wall (~30 cm) and inward if outer wall <20 cm
